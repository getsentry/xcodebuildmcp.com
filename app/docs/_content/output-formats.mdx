...
```json
{
  ...
  "nextSteps": [
    "Build an app for this simulator: xcodebuildmcp simulator build --simulator-id A1B2C3D4-E5F6-7890-ABCD-EF0123456789"
  ]
}
```
...
```shell
xcodebuildmcp ui-automation snapshot-ui --simulator-id <UDID> --output json --verbose
```
...
```shell
xcodebuildmcp simulator build --scheme CalculatorApp --workspace-path example_projects/iOS_Calculator/CalculatorApp.xcworkspace --output json --style minimal
```
...
```json
{
  ...
  "nextSteps": [
    "Run the app on this simulator: xcodebuildmcp simulator build-run --scheme CalculatorApp --workspace-path example_projects/iOS_Calculator/CalculatorApp.xcworkspace"
  ]
}
```
