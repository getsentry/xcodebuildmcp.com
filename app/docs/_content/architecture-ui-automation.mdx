...
<PageHeader
  ...
  title="UI Automation"
  lede="How XcodeBuildMCP turns raw accessibility output into stable runtime snapshots, safe actions, post-action captures, and agent-oriented next-step guidance."
/>

UI automation is the part of XcodeBuildMCP where an agent can inspect an app, choose elements, and perform gestures without shelling out to Simulator or AXe directly. The contributor goal is not to make one app easy to drive. The goal is to preserve an app-agnostic model of what the UI exposes, what actions are valid, and which follow-up actions are most useful.
...
- **runtime snapshot**: XcodeBuildMCP's compact, public, model-facing UI state. It contains refs, roles, frames, actions, text, screen hash, and sequence.
- **element ref**: A short per-snapshot identifier such as `e18`. It is only valid for the snapshot sequence that produced it.
- **runtime snapshot store**: The in-memory per-simulator record that maps refs back to private metadata and raw nodes for action execution.
...
flowchart TD
    ...
    SnapshotBuilder --> PublicSnapshot[Compact runtime snapshot]
    PublicSnapshot --> StructuredOutput[Structured output envelope]
...
### 2. Render compact output

The public snapshot is compacted before it reaches agents. Instead of returning full raw nodes, output groups elements into lists such as:
...
- Do not include raw nodes in public output.
...
Most UI actions attempt a post-action capture. This gives the agent a fresh screen hash, fresh refs, and fresh next steps without requiring an immediate extra `snapshot_ui` call.

Business rules:

- Post-action capture is an optimization, not a license to reuse stale refs.
...
- Application or window roots can get a special swipe frame when a sheet grabber descendant indicates a sheet-style gesture region.
...
Scroll next steps currently choose the first scrollable element left after foreground filtering.
...
- Competing scroll views inside the filtered region are not yet ranked by specificity, identifier quality, visible area, or relationship to foreground content. This can make an anonymous ancestor scroll win over a more obvious sheet scroll.
...
The UI automation tools return structured output under the normal XcodeBuildMCP envelope.
